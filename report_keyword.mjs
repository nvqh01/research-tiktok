...
import { KeywordWorker } from "./worker.mjs";

const INDEX = 1;
const MAX_PROCESSED_REQUESTS = null;
...
async function start(index) {
  ...
  const worker = new KeywordWorker(workerOptions, inputManager, proxyManager);
  await worker.start();

  const statistics = worker.getStatistics();

  appendFileSync(
    `reports/keyword/index_${index}.txt`,
...
function getOptions(index) {
  ...
  let workerOptions = {
    endpoint: ENDPOINT_FOR_KEYWORD,
    headers: DEFAULT_HEADERS_FOR_KEYWORD,
    params: DEFAULT_PARAMS_FOR_KEYWORD,
  ...
  const proxies = PROXIES_FOR_KEYWORD.splice(0, TOTAL_PROXIES);
  ...
  return {
    listInputs: KEYWORDS.splice(0, TOTAL_INPUTS).map((keyword) => {
      return {
        input: keyword,
        nextPageToken: "0",
      };
